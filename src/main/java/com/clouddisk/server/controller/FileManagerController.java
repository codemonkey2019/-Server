...
public class FileManagerController {
    ...
    public void search(Socket socket, MessageBody messageBody) {
        ...
        //传给fileManagerService处理
        SearchAnswer answer = fileManagerService.search(request);
        log.info(answer.toString());
        //给客户端返回数据
        SocketConnect.sendAnwswerToClient(socket,answer);
    }

    public void searchAll(Socket socket) {
        SearchAnswer answer = fileManagerService.searchAll();
        log.info(answer.toString());
        //给客户端返回数据
        SocketConnect.sendAnwswerToClient(socket,answer);
    }

    public void download(Socket socket, MessageBody messageBody) {
        DownLoadRequest request = InformationCast.messageBodyToRequestBody(messageBody,DownLoadRequest.class);
        fileManagerService.sendFiles(request.getSelectedFiles(),socket);
    }
}
